...
def add_book():
  ...
  sql = "insert into book(title,author,price,pages,publisher,edition,status) values ( '{}','{}',{},{},'{}',{},'{}')".format(title,author,price,pages,publisher,edition,'available')
...
def add_member():
  ...
  sql = "insert into member(name,class,address,phone,email) values ('{}','{}','{}','{}','{}')".format(name,clas,address,phone,email)
...
def mem_issue_status(mem_id):
    conn = mysql.connector.connect(
        host='localhost', database='dj1', user='root',port='3307',password='dj123')
    cursor = conn.cursor()
    sql ="select * from transaction where m_id = {} and dor is NULL".format(mem_id)
    #print(sql)
    cursor.execute(sql)
...
def book_issue_status(book_id,mem_id):
    conn = mysql.connector.connect(
        host='localhost', database='dj1',port='3307',user='root', password='dj123')
    cursor = conn.cursor()
    sql = "select * from transaction where b_id = {} and m_id = {} and dor is NULL".format(book_id,mem_id)
    cursor.execute(sql)
    result = cursor.fetchone()
    return result
...
def issue_book():
    ...
    if len(result1) == 0:
      if result == 'available':
          sql = "insert into transaction(b_id, m_id, doi) values({},{},'{}')".format(book_id,mem_id,str(today))
          sql_book = "update book set status= '{}' where id = {}".format('issue',book_id)
          cursor.execute(sql)
      ...
      if len(result1)<1:
        ...
        sql_book = "update book set status = issue where id = {}".format(book_id)
...
def return_book():
    ...
    if result==None:
       ...
       sql="update book set status = '{}' where id ={}".format('available',book_id)
       din = (today - result[3]).days
       fine = din * fine_per_day    #  fine per data
       sql1 = "update transaction set dor ='{}' , fine={} where b_id={} and m_id={} and dor is NULL".format(str(today),str(fine),book_id,mem_id) 
